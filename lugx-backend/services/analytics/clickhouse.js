...
class ClickHouseService {
  constructor() {
    ...
    console.log(
      `   Password length: ${config.password ? config.password.length : 0}`
    );
    ...
    if (hasRealCredentials) {
      console.log(
        "Real credentials detected, attempting ClickHouse connection"
      );
      this.client = createClient(config);
      this.initializeDatabase();
    } else {
      this.logWithIST(
        "ClickHouse: No real credentials provided, running in mock mode"
      );
      this.isConnected = false;
    }
  }
  ...
  async initializeDatabase() {
    try {
      ...
      this.logWithIST(
        "ClickHouse analytics database and tables initialized successfully"
      );
    } catch (error) {
      this.logWithIST(
        `❌ Error initializing ClickHouse database: ${error.message}`,
        true
      );
      this.isConnected = false;
    }
  ...
  async insertPageView(data) {
    if (this.debugMode) {
      this.logWithIST(
        `🔍 DEBUG: Received page view data: ${JSON.stringify(data, null, 2)}`
      );
    }

    if (!this.isConnected) {
      this.logWithIST("ClickHouse not connected, skipping page view insert");
      return;
    }
    ...
    try {
      ...
      this.logWithIST(
        `❌ Error inserting page view to ClickHouse: ${error.message}`,
        true
      );
    }
  }
  ...
  async insertClickEvent(data) {
    if (this.debugMode) {
      this.logWithIST(
        `🔍 DEBUG: Received click event data: ${JSON.stringify(data, null, 2)}`
      );
    }

    if (!this.isConnected) {
      this.logWithIST("ClickHouse not connected, skipping click event insert");
      return;
    }
    ...
    try {
      ...
      this.logWithIST(
        `❌ Error inserting click event to ClickHouse: ${error.message}`,
        true
      );
    }
  }
  ...
  async insertScrollDepth(data) {
    if (this.debugMode) {
      this.logWithIST(
        `🔍 DEBUG: Received scroll depth data: ${JSON.stringify(data, null, 2)}`
      );
    }

    if (!this.isConnected) {
      this.logWithIST("ClickHouse not connected, skipping scroll depth insert");
      return;
    }
    ...
    try {
      ...
      this.logWithIST(
        `❌ Error inserting scroll depth to ClickHouse: ${error.message}`,
        true
      );
    }
  }
  ...
  async insertSession(data) {
    if (this.debugMode) {
      this.logWithIST(
        `🔍 DEBUG: Received session data: ${JSON.stringify(data, null, 2)}`
      );
    }

    if (!this.isConnected) {
      this.logWithIST("ClickHouse not connected, skipping session insert");
      return;
    }
    ...
    try {
      ...
      this.logWithIST(
        `❌ Error inserting session to ClickHouse: ${error.message}`,
        true
      );
    }
  }
  ...
  async insertCustomEvent(data) {
    if (this.debugMode) {
      this.logWithIST(
        `🔍 DEBUG: Received custom event data: ${JSON.stringify(data, null, 2)}`
      );
    }

    if (!this.isConnected) {
      this.logWithIST("ClickHouse not connected, skipping custom event insert");
      return;
    }
    ...
    try {
      ...
      this.logWithIST(
        `❌ Error inserting custom event to ClickHouse: ${error.message}`,
        true
      );
    }
  }
  ...
  async getRealtimePageViews(minutes = 30) {
    try {
      if (!this.isConnected) {
        this.logWithIST("ClickHouse not connected, returning empty page views");
        return [];
      }
      ...
      this.logWithIST(
        `❌ Error fetching realtime page views: ${error.message}`,
        true
      );
      return [];
    }
  ...
  async getRealtimeClickHeatmap(pageUrl, minutes = 30) {
    try {
      if (!this.isConnected) {
        this.logWithIST(
          "ClickHouse not connected, returning empty click heatmap"
        );
        return [];
      }
      ...
      this.logWithIST(
        `❌ Error fetching click heatmap: ${error.message}`,
        true
      );
      return [];
    }
  ...
  async getScrollAnalytics(pageUrl, minutes = 30) {
    try {
      if (!this.isConnected) {
        this.logWithIST(
          "ClickHouse not connected, returning empty scroll analytics"
        );
        return [];
      }
      ...
      this.logWithIST(
        `❌ Error fetching scroll analytics: ${error.message}`,
        true
      );
      return [];
    }
  ...
  async getDashboardData(hours = 24) {
    try {
      if (!this.isConnected) {
        this.logWithIST(
          "ClickHouse not connected, returning empty dashboard data"
        );
        return null;
      }

      const [pageViews, sessions, topPages, deviceBreakdown] =
        await Promise.all([
  ...
  async getDataForMySQLSync(table, lastSyncTime) {
    try {
      ...
      this.logWithIST(
        `❌ Error fetching data for MySQL sync: ${error.message}`,
        true
      );
      return [];
    }
